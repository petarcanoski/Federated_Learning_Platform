...
class ExperimentService:
    ...
    def _serialize_state(self, state: ExperimentHospitalState) -> HospitalStateOut:
        return HospitalStateOut(
            ...
            hospital_code=state.hospital_code,
            ...
            client_metrics=[
                {
                    ...
                    "hospital_name": metric.hospital_name,
    ...
    def _find_experiment(self, job_id: str) -> Experiment:
        experiment = Experiment.objects(job_id=job_id).first()
        if experiment is None:
            raise KeyError(job_id)
        return experiment
    def _find_hospital_state(self, experiment: Experiment, hospital_id: int) -> ExperimentHospitalState | None:
        return next((state for state in experiment.hospital_states if state.hospital_id == hospital_id), None)
    def _legacy_round(self, experiment: Experiment) -> ExperimentOut:
    ...
    def _healthcare_round(self, experiment: Experiment, send_immediately: bool = True) -> ExperimentOut:
        hospital_states = [state for state in experiment.hospital_states if Hospital.objects(id=state.hospital_id, is_active=True).first() is not None]
        ...
        for state in hospital_states:
            ...
            state.notification = f"Training local model for {state.hospital_name}..."
            hospital = Hospital.objects(id=state.hospital_id).first()
        ...
        for index, state in enumerate(hospital_states):
            dataset, stats = build_hospital_dataset(experiment.disease_type, state.hospital_name)
            ...
            if hospital is not None:
                ...
                hospital.disease_type = experiment.disease_type
        ...
        global_loss, global_accuracy = self._evaluate_state(
            experiment.current_weights,
            TensorDataset(torch.cat(all_dataset_features, dim=0), torch.cat(all_dataset_labels, dim=0)),
            self._hidden_dim(experiment),
            spec.input_dim,
        )
        ...
        if privacy_cfg.enabled and privacy_cfg.noise_multiplier > 0:
            ...
            experiment.dp_delta = privacy_cfg.delta
        if send_immediately:
            self._broadcast_current_model(experiment)
        else:
            experiment.save()
        return self._serialize_experiment(experiment)
    def _broadcast_current_model(self, experiment: Experiment) -> list[str]:
        notifications = []
        for state in experiment.hospital_states:
            if state.raw_weights_json is None:
                continue
            spec = get_disease_spec(experiment.disease_type)
            dataset, _ = build_hospital_dataset(experiment.disease_type, state.hospital_name)
    ...
    def create_experiment(self, request: ExperimentCreateRequest, creator: User | None = None) -> ExperimentOut:
        ...
        if healthcare:
            ...
            hospitals = list(Hospital.objects(code__in=selected_codes).order_by("id"))
            if not hospitals:
                hospitals = list(Hospital.objects.order_by("id"))
            for hospital in hospitals:
                dataset, stats = build_hospital_dataset(disease_type, hospital.name)
                experiment.hospital_states.append(
    ...
    def get_experiment(self, job_id: str) -> ExperimentOut:
        self._ensure_seeded()
        return self._serialize_experiment(self._find_experiment(job_id))
    def run_round(self, job_id: str) -> ExperimentOut:
        self._ensure_seeded()
        experiment = self._find_experiment(job_id)
        if experiment.status == "finished":
            return self._serialize_experiment(experiment)
        if self._is_healthcare_experiment(experiment):
            return self._healthcare_round(experiment, send_immediately=True)
        return self._legacy_round(experiment)
    ...
    def me(self, user: User) -> MeOut:
        self._ensure_seeded()
        fresh_user = User.objects(id=user.id).first()
        if fresh_user is None:
            raise KeyError("user not found")
        hospital = Hospital.objects(id=fresh_user.hospital_id).first() if fresh_user.hospital_id is not None else None
    ...
    def create_hospital_account(self, request: HospitalCreateRequest, current_user: User) -> dict[str, Any]:
        ...
        self._ensure_seeded()
        if Hospital.objects(code=request.code).first() is not None:
    ...
    def list_hospitals(self, current_user: User) -> list[HospitalOut]:
        ...
        self._ensure_seeded()
        return [self._serialize_hospital(hospital) for hospital in Hospital.objects.order_by("id")]
    def get_admin_dashboard(self, current_user: User) -> AdminDashboardOut:
        ...
        experiments = list(Experiment.objects.order_by("-created_at"))
        all_states: list[ExperimentHospitalState] = []
        for experiment in experiments:
            all_states.extend(experiment.hospital_states)
        return AdminDashboardOut(
            admin=self._serialize_user(db_user or current_user),
            hospitals=[self._serialize_state(state) for state in all_states],
            experiments=[
                {
                    "job_id": exp.job_id,
                    "status": exp.status,
    ...
    def get_hospital_dashboard(self, current_user: User) -> HospitalDashboardOut:
        ...
        if user is None or user.hospital_id is None:
            raise KeyError("hospital not found")
        hospital = Hospital.objects(id=user.hospital_id).first()
        if hospital is None:
            raise KeyError("hospital not found")
        experiment = None
        state = None
        for candidate in Experiment.objects.order_by("-created_at"):
            matched = self._find_hospital_state(candidate, hospital.id)
        ...
        if experiment is not None and state is not None:
            experiment_payload = {
                "job_id": experiment.job_id,
                "status": experiment.status,
                "round_progress": experiment.round_progress,
    ...
    def train_hospital_local_model(self, current_user: User, job_id: str | None = None) -> HospitalDashboardOut:
        ...
        if hospital is None:
            raise KeyError("hospital not found")
        experiment = self._find_experiment(job_id) if job_id else None
        if experiment is None:
            experiment = next((exp for exp in Experiment.objects.order_by("-created_at") if self._find_hospital_state(exp, hospital.id) is not None), None)
        if experiment is None:
            raise KeyError("experiment not found")
        state = self._find_hospital_state(experiment, hospital.id)
        if state is None:
    ...
    def admin_broadcast_model(self, current_user: User, job_id: str) -> BroadcastResult:
        ...
        self._ensure_seeded()
        experiment = self._find_experiment(job_id)
        notifications = self._broadcast_current_model(experiment)
